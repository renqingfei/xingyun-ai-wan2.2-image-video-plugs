...
def _wait_for_video_history(
    ...
    *,
    ...
    while time.time() < deadline:
        data = _get_json(history_url, timeout_s=120)
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            upload_resp = upload_image_to_comfyui(comfyui_url, local_image_path)
            ...
            self._send_json(
                502,
                {"error": {"message": f"upload image failed: {e}", "type": "upstream_error"}},
            )
            return
        ...
        try:
            resp = _post_json(urljoin(comfyui_url, "api/prompt"), payload)
        except (HTTPError, URLError, TimeoutError) as e:
            self._send_json(502, {"error": {"message": f"upstream comfyui failed: {e}", "type": "upstream_error"}})
            return
        ...
        try:
            history_item = _wait_for_video_history(
                ...
                timeout_s=self.server.poll_timeout_s,
            ...
            self._send_json(
                502,
                {"error": {"message": f"fetch result failed: {e}", "type": "upstream_error", "prompt_id": prompt_id}},
            )
            return
